...
function get_items(){
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_baiviet SET tinnoibat =1 WHERE  id = ".$id_up."";
...
$sql.=" where  ten like '%".$_POST['search']."%'";
	...
	if($_REQUEST['id_cat']!='')$url="index.php?com=baiviet&act=man&id_cat=".$_REQUEST['id_cat'];
	else $url="index.php?com=baiviet&act=man";
	$maxR=25;
	$maxP=4;
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		} redirect("index.php?com=baiviet&act=man");} else transfer("Không nhận được dữ liệu", "index.php?com=news&act=man");
}
//===========================================================
function get_cats(){
	global $d, $items, $paging;
	...
	$curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
	$url="index.php?com=baiviet&act=man_cat";
	$maxR=20;
	$maxP=4;
	$paging=paging($items, $url, $curPage, $maxR, $maxP);
	$items=$paging['source'];
...
function get_cat(){
	...
	$id = isset($_GET['id']) ? themdau($_GET['id']) : "";
	if(!$id)
	transfer("Không nhận được dữ liệu", "index.php?com=baiviet&act=man_cat");
	
	$sql = "select * from #_baiviet_item where id='".$id."'";
	$d->query($sql);
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=baiviet&act=man_cat");
	$item = $d->fetch_array();
}
...
function save_cat(){
	...
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=baiviet&act=man_cat");
	$id = isset($_POST['id']) ? themdau($_POST['id']) : "";
	if($id){
		$data['ten'] = $_POST['ten'];
		$data['tenkhongdau'] = changeTitle($_POST['ten']);
		...
		$d->setWhere('id', $id);
		if($d->update($data))
			redirect("index.php?com=baiviet&act=man_cat&curPage=".$_REQUEST['curPage']."");
		else
			transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=baiviet&act=man_cat");
	}else{
		$data['ten'] = $_POST['ten'];
		$data['tenkhongdau'] = changeTitle($_POST['ten']);
		$data['stt'] = $_POST['stt'];	
		...
		$d->setTable('news_item');
		if($d->insert($data))
			redirect("index.php?com=baiviet&act=man_cat");
		else
			transfer("Lưu dữ liệu bị lỗi", "index.php?com=baiviet&act=man_cat");
	}
}
...
function delete_cat(){
	...
	if(isset($_GET['id'])){
		...
		if($d->query($sql))
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=baiviet&act=man_cat");
	}else transfer("Không nhận được dữ liệu", "index.php?com=baiviet&act=man_cat");
}
?>