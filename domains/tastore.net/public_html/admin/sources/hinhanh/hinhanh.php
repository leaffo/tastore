...
function get_photos(){
	...
	$id_photo=$_REQUEST['idc'];
	 ...
	 $sql = "select * from #_hinhanh where id_photo='".$id_photo."' order by stt,id desc ";			
	$d->query($sql);
	$items = $d->result_array();
	
	$curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
	$url="index.php?com=hinhanh&act=man&idc=".$_REQUEST['idc']."";
	$maxR=10;
	$maxP=4;
...
function get_photo(){
	...
	if(!$id)
		transfer("Không nhận được dữ liệu", "index.php?com=hinhanh&act=man&idc=".$_REQUEST['idc']."");
	
	$d->setTable('hinhanh');
	$d->setWhere('com', 'hinhanh');
	$d->setWhere('id', $id);
	$d->select();
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=hinhanh&act=man&idc=".$_REQUEST['idc']."");
	$item = $d->fetch_array();
	$d->reset();
}
...
function save_photo(){
	...
	if($id){ // cap nhat
			...
			if($photo = upload_image("file", 'jpg|png|gif', _upload_hinhanh,$file_name)){
				$data['photo'] = $photo;
				$data['thumb'] = create_thumb($data['photo'], 600, 600, _upload_hinhanh,$file_name);
				$d->setTable('hinhanh');
				$d->setWhere('id', $id);
				$d->select();
				if($d->num_rows()>0){
			...
			for($i=0; $i<5; $i++){
						...
						if($photo = upload_image("file".$i, 'jpg|png|gif', _upload_hinhanh,$file_name.$i))
					{
						...
						$d->setTable('hinhanh');
						if(!$d->insert($data)) transfer("Lưu dữ liệu bị lỗi", "index.php?com=hinhanh&act=man&idc=".$_REQUEST['idc']."");
						}
			}
...
function delete_photo(){
	...
	if(isset($_GET['id'])){
		...
		if($d->delete())
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=hinhanh&act=man&idc=".$_REQUEST['idc']."");
	}else transfer("Không nhận được dữ liệu", "index.php?com=hinhanh&act=man&idc=".$_REQUEST['idc']."");
}
