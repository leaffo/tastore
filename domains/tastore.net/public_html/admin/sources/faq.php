...
function get_items(){
	...
	$sql_sp = "SELECT id,tinnoibat FROM table_faq where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	...
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_faq SET tinnoibat =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_faq SET tinnoibat =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	{
	$id_up = @$_REQUEST['hot'];
	$sql_sp = "SELECT id,hot FROM table_faq where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	$hienthi=$cats[0]['hot'];
	//echo "id:". $spdc1;
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_faq SET hot =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_faq SET hot =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	...
	if($_REQUEST['id_cat']!='')$url="index.php?com=faq&act=man&id_cat=".$_REQUEST['id_cat'];
	else $url="index.php?com=faq&act=man";
	$maxR=25;
	$maxP=4;
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		} redirect("index.php?com=faq&act=man");} else transfer("Không nhận được dữ liệu", "index.php?com=news&act=man");
}
//===========================================================
function get_cats(){
	global $d, $items, $paging;
	$sql = "select * from #_faq_item order by stt";
	$d->query($sql);
	$items = $d->result_array();
