...
function get_items(){
	...
	if($_REQUEST['id_cat']!='')$url="index.php?com=tinhthanh&act=man&id_cat=".$_REQUEST['id_cat'];
	else $url="index.php?com=tinhthanh&act=man";
	$maxR=25;
	$maxP=4;
...
function delete_item(){
	...
	if(isset($_GET['id'])){
		...
		} redirect("index.php?com=tinhthanh&act=man");} else transfer("Không nhận được dữ liệu", "index.php?com=news&act=man");
}
//===========================================================
function get_cats(){
	global $d, $items, $paging;
	$sql = "select * from #_tinhthanh_item order by stt";
	$d->query($sql);
	$items = $d->result_array();
